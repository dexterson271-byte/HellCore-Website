 ...
 */
public final class MysteryDustCommand implements CommandExecutor, TabCompleter {

    private static final List<String> SUB = Collections.singletonList("add");

    private final BWMystery plugin;

    public MysteryDustCommand(BWMystery plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (!sender.hasPermission("bwmystery.command.mysterydust")) {
        ...
        sender.sendMessage(ChatColor.GREEN + "Added "
                + ChatColor.YELLOW + amount + ChatColor.GREEN + " mystery dust to "
                + ChatColor.YELLOW + targetName + ChatColor.GREEN
                + " (new total: " + profile.getDust() + ").");

        if (online != null) {
            online.sendMessage(ChatColor.LIGHT_PURPLE + "You received "
                    + ChatColor.YELLOW + amount
                    + ChatColor.LIGHT_PURPLE + " mystery dust.");
        }
        return true;
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String alias, String[] args) {
        ...
        if (args.length == 3) return filter(Arrays.asList("10", "100", "1000"), args[2]);
        return Collections.emptyList();
    }
