...
public class ApiKeyManager {
    ...
    public boolean init() {
        try {
            ...
            try (ResultSet rs = conn.createStatement().executeQuery(
                    "SELECT uuid, api_key, banned FROM api_keys")) {
                while (rs.next()) {
    ...
    public int getRateLimit(UUID uuid) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(
            ...
            ps.setString(1, key);
            try (ResultSet rs = ps.executeQuery()) { if (rs.next()) return rs.getInt("rate_limit"); }
    ...
    public long getCreatedAt(UUID uuid) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(
            ...
            ps.setString(1, key);
            try (ResultSet rs = ps.executeQuery()) { if (rs.next()) return rs.getLong("created_at"); }
        } catch (SQLException ignored) {}
        return 0;
    }
    ...
    public long getTotalRequestsAllKeys() {
        try (ResultSet rs = conn.createStatement().executeQuery(
                "SELECT SUM(total) as t FROM rate_limits")) {
